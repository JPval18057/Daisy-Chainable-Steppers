...
#define CommsBaudrate 500000 //1,000,000
...
#define hall_switch 8  //10 is the default
...
const double PositionControlResolution = (360/(steps_per_revolution*EEPROM_GEAR_RATIO)); //Smallest angle possible
...
double DesiredSpeed = 50; //Desired speed (rpm)
...
//TIMER VARIABLES
const unsigned long interval = 100; //In ms
const double interval_seg = interval/1000; //interval in seconds
...
void setup()
{
  ...
  RotationSpeed_previous = displaycounter; //The first display counter value
...
void loop()
{
  ...
  //**********************************************************************************
    ...
    case READ_POSITION:
      ...
      JSONSEND(READ_POSITION, MASTER_ID, angle);
      //Change command so that it does this only once
      command = CMD_NULL;
      break;
    ...
    case READ_SPEED:
      ...
      JSONSEND(READ_SPEED,MASTER_ID,RotationSpeed);
      //Change command so that it does this only once
      command = CMD_NULL;
      break;
    ...
    case READ_KP:
      ...
      if (EEPROM_CONTROL_MODE==POSITION)
      {
        JSONSEND(READ_KP,MASTER_ID,kp);
      } else
      {
        JSONSEND(READ_KP,MASTER_ID,speed_kp);
      }
    ...
    case READ_KI:
      ...
      if (EEPROM_CONTROL_MODE==POSITION)
      {
        JSONSEND(READ_KI,MASTER_ID,ki);
      } else
      {
        JSONSEND(READ_KI,MASTER_ID,speed_ki);
      }
      //Change command so that it does this only once
      command = CMD_NULL;
      break;
    ...
    case READ_KD:
      ...
      if (EEPROM_CONTROL_MODE==POSITION)
      {
        JSONSEND(READ_KD,MASTER_ID,kd);
      } else
      {
        JSONSEND(READ_KD,MASTER_ID,speed_kd);
      }
      //Change command so that it does this only once
      command = CMD_NULL;
      break;
    ...
    case READ_GEAR_RATIO:
      ...
      JSONSEND(READ_GEAR_RATIO,MASTER_ID,EEPROM_GEAR_RATIO);      
      //Change command so that it does this only once
      command = CMD_NULL;
      break;
...
void TimerHandler()
{
  ...
  */
  ...
  //Speed measurement
  RotationSpeed_current = displaycounter;
  RotationSpeed = (RotationSpeed_current - RotationSpeed_previous)/(interval_seg); //invterval is in ms
  RotationSpeed_previous = RotationSpeed_current; //Update the number of steps taken in this interrupt
  ...
  } else 
  {
    ...
    MotorSpeed = PID(speed_kp,speed_kd,speed_ki,DesiredSpeed_steps, RotationSpeed,speed_err_kd,speed_err_ki,speed_err_prev);
  }
  Serial.print("\n");
